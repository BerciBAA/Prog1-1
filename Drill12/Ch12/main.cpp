...
int main()
{
	...
	Polygon polly;
	polly.add(Point{300,222});
	polly.add(Point{350,32});
	polly.add(Point{500,532});
	polly.set_fill_color(Color::red);
	ostringstream oss;
	oss << "SCREEEN" << x_max() << "*" << y_max() << " WINDOWS" << win.x_max() << "*" << win.y_max();
	Text kacsa{Point{500,500}, oss.str()};

 
	Mark m (Point{231,231},'x');
	win.attach(m);
	win.attach(kacsa);
	//swin.attach(ii);
	win.attach(polly);
